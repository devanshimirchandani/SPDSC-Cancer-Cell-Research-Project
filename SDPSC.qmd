...
CrC_counts_subset1 <- gene_data[, rownames(CrC_metadata_subset1)]
crc_seurat1 <- CreateSeuratObject(counts = CrC_counts_subset1, meta.data = CrC_metadata_subset1)

# Subset for sample 2
CrC_metadata_subset2 <- subset(metadata, orig.ident == "F007")
CrC_counts_subset2 <- gene_data[, rownames(CrC_metadata_subset2)]
crc_seurat2 <- CreateSeuratObject(counts = CrC_counts_subset2, meta.data = CrC_metadata_subset2)

# Subset for sample 3
CrC_metadata_subset3 <- subset(metadata, orig.ident == "CRC1_8810")
CrC_counts_subset3 <- gene_data[, rownames(CrC_metadata_subset3)]
crc_seurat3 <- CreateSeuratObject(counts = CrC_counts_subset3, meta.data = CrC_metadata_subset3)


crc_seurat1 <- NormalizeData(crc_seurat1)
crc_seurat2 <- NormalizeData(crc_seurat2)
crc_seurat3 <- NormalizeData(crc_seurat3)

crc_seurat1 <- FindVariableFeatures(crc_seurat1, selection.method = "vst", nfeatures = 2000)
crc_seurat2 <- FindVariableFeatures(crc_seurat2, selection.method = "vst", nfeatures = 2000)
crc_seurat3 <- FindVariableFeatures(crc_seurat3, selection.method = "vst", nfeatures = 2000)
...
# Optionally, create heatmaps or other visualizations
top10_integrated <- head(VariableFeatures(crc_integrated), 10)
DoHeatmap(crc_integrated, features = top10_integrated) + NoLegend()
